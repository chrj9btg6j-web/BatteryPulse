...
- 將充電上限快捷方案集中到總覽的「充電上限」卡片，依韌體實際回報顯示可用選項與自訂套用控制。
- 移除電池健康頁與設定頁的重複快捷控制，保留電池健康頁的上限狀態與來源資訊。
- 增加總覽圖塊的垂直間距，並讓充電上限卡片依完整控制內容撐開。
- 將總覽摘要格固定為預留最大數值的尺寸，避免數值更新時推動其他格子。
- 總覽新增 CPU／GPU 即時使用率摘要，並縮短摘要狀態文字。
